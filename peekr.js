...
cachingIfrm.visibility = 'hidden';

previewGlowingBorderContainer.classList.add('peekr_loading_border_container');
...
previewIfrm.addEventListener('animationend', ev => {
  ...
  if (ev.animationName.match(/Out/)) previewIfrm.style.display = 'none';
...
cachingIfrm.addEventListener('load', ev => {
  if (cachingIfrm.src != 'about:blank') {
