...
let backupBodyStyle = '';
const iframeSandbox = 'allow-forms allow-modals allow-orientation-lock allow-pointer-lock allow-popups allow-pr' +
    'esentation allow-same-origin allow-scripts';
...
const closeModal = _ => {
  ...
  setTimeout(() => {
    previewOvrly
      .classList
      .add('pkr_anim', 'overlayOut');
  }, 200);

  previewIfrm
    .classList
    .add('iframeOut', 'pkr_anim');
  window.focus();
  return true;
};
...
const handlePreviewLoaded = ev => {
  ...
  previewGlowingBorderContainer
    .classList
    .add('fadeOut', 'pkr_anim');
  const html = document.querySelector('html');
  previewOvrly
    .classList
    .add('pkr_anim', 'overlayIn');
  backupBodyStyle     = html.style.overflow;
  html.style.overflow = 'hidden';
  setTimeout(() => {
    previewIfrm
      .classList
      .add('pkr_anim', 'iframeIn');
  }, 200);
};

const modalOnScreen = _ => {
  return previewIfrm.style.display == 'block' && previewIfrm.src && previewIfrm
    .src
    .trim() != '' && previewIfrm.src != 'about:blank'
      ? true
      : false;
};
//shift state
const modUpHandler = e => {
  if (e.keyCode == 16) {
    ...
    if (modalOnScreen()) 
      chrome
        .runtime
        .sendMessage({openInWindow: previewIfrm.src});
    else {
      chrome
        .runtime
        .sendMessage({modState: 'keyup'});
      modState = 'up';
    }
  }
};
...
const modDownHandler = e => {
  if (e.keyCode == 16) {
    chrome
      .runtime
      .sendMessage({modState: 'keydown'});
    window.removeEventListener('keydown', modDownHandler);
    modState = 'down';
  }
};
// window.addEventListener('keydown', modDownHandler); window.addEventListener('keyup',
// modUpHandler); escape
...
const showGlowingBorder = linkElement => {
  ...
  console.log(
    `
    top: ${Math.floor(linkElement.offsetTop) - mrgn}px;
  left: ${Math.floor(linkElement.offsetLeft) - mrgn}px;
...
document
  .head
  .appendChild(previewStyles);
document
  .body
  .appendChild(previewOvrly);
document
  .body
  .appendChild(previewIfrm);

previewGlowingBorderContainer.addEventListener('animationend', ev => {
  previewGlowingBorderContainer
    .classList
    .remove('pkr_anim', ev.animationName);
  previewGlowingBorderContainer.style.display = ev
    .animationName
    .match(/Out/)
      ? 'none'
      : 'block';
...
document
  .body
  .appendChild(previewGlowingBorderContainer);
document
  .body
  .appendChild(cachingIfrm);
